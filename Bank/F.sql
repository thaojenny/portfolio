-------------------------------update formular to calculate inventory/quick ratio------------------------------------------------------
select '2021' as [Năm],
		[Masothue] [Mã số thuế],
		[TenDN] [ Tên DN],
		gso.[Mã ngành C5],
...
--------check distribution- delete outliers------------------------
with npp as
(
select '2021' as [Năm],
		[Masothue] [Mã số thuế],
...
left join [practice].[dbo].[FB map] [fb]
on gso.[Mã ngành C5] =fb.[Mã ngành C5]
where [Cấp 2] like '%FMCG%' and fb.[Tiểu ngành FMCG] is not null

)
select distinct([Nhóm con]) as [PK],
		percentile_cont(0.25) within group (order by [VQKPTRA]) over(partition by [Nhóm con]) as [25th],
		percentile_cont(0.5) within group (order by [VQKPTRA]) over(partition by [Nhóm con]) as [50th],
		percentile_cont(0.75) within group (order by [VQKPTRA]) over(partition by [Nhóm con]) as [75th]
from npp 
where [Nhóm] = 'NPP'
